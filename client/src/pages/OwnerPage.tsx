...
function KeysTab() {
  ...
  const filtered = (Object.entries(keys) as [string, KeyRecord][]).filter(([k, r]) =>
  ...
  return (
    <div className="owner-tab">
      ...
      {editKey && keys[editKey] && (
        <EditKeyModal keyId={editKey} rec={keys[editKey]} onClose={() => setEditKey(null)} onSave={saveKey} />
      )}
      ...
      {loading ? (
        ...
        <div className="keys-table-wrap">
          <table className="keys-table">
            <thead>
              <tr>
                <th>Key</th><th>Username</th><th>Duration</th>
                <th>Status</th><th>HWID</th><th>Registered</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                ...
                <tr key={k}>
                  ...
                  <td>
                    <div className="key-actions">
                      ...
                      <button className="btn-action" onClick={() => saveKey(k, { disabled: !r.disabled })}>
...
function DownloadTab() {
  ...
  useEffect(() => {
    fetch("/api/owner/download", { credentials: "include" })
      .then(r => r.json()).then(d => { setInput(d.url || ""); setCurrent(d.url || ""); });
  }, []);
  ...
  return (
    <div className="owner-tab">
      ...
      <p className="owner-tab-sub">Set the download URL for the Forsaken <code style={{ fontFamily: "monospace", color: "var(--gray-200)" }}>.rar</code>. Customers see this via <code style={{ fontFamily: "monospace", color: "var(--gray-200)" }}>/download</code> and the dashboard.</p>
...
function AnalyticsTab() {
  ...
  const active   = vals.filter(r => r.hwid !== null).length;
  ...
  return (
    <div className="owner-tab">
      ...
      <div className="analytics-grid">
        {[["Total Keys", total], ["Claimed", claimed], ["Active (HWID)", active], ["Disabled", disabled], ["Unclaimed", unclaimed]].map(([l, v]) => (
...
function DangerTab() {
  ...
  const verify = async () => {
    setError("");
    const r = await fetch("/api/owner/danger/verify", {
      method: "POST", credentials: "include",
      headers: { "Content-Type": "application/json" },
      ...
      <div className="danger-keys-form">
        {[0, 1, 2].map(i => (
          <div key={i} className="form-group">
            ...
            <input className="form-input" type="password" value={inputs[i]}
      ...
      <div className="danger-section">
        ...
        <div className="danger-owners">
          ...
          {owners.extra.map(id => (
            <div key={id} className="danger-owner-row">
              <code>{id}</code>
              <button className="btn btn-danger btn-sm" onClick={() => removeOwner(id)}>Remove</button>
            </div>
          ))}
        </div>
